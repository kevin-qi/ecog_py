...
def load_ecog(file_path, fs, fsds = 3000, delimiter='\t'):
	...
	fsds (float)
		...
		Must be atleast twice the highest frequency you want to analyze. Note that twice the nyquist frequency is the theoretical minimum, so it is recommended to go higher to have some wiggle room.
	
	delimiter (optional, only needed for csv formats)
...
def load_trial_timings(file_path, fs, fsds = 3000, delimiter='\t'):
	...
	Returns
	-------
	sweep_start (num_samples,)
		Square wave (high during trial, low in between trial)
		Downsampled to fsds (Must match fsds used for loading ECoG data!!!)

	trial_indices (num_trials, 2)
